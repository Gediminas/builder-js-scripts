...
//***************************************************
// Build *.sln using Microsoft Visual Studio C++ 16.0
//***************************************************

function ReadProjectConfigurationsVC19($path_dsp)
{
	$aCfg = array();
...
{
	return ReadProjectConfigurationsVC19($path_dsp);
}

function GetOpenPrjCommand($prj)
{
	$devenv = 'c:\Program Files (x86)\Microsoft Visual Studio\2019\Professional\Common7\IDE\devenv';
...
function GetBuildCommands($build_info, $rebuild, $log_file)
{
	...
	if( !empty($platform) ) {
		$cmds[0] = "%COMSPEC% /c \"\"{$exe}\" \"{$sln}\" /m /v:m /t:{$build} /p:PlatformToolset=v142 /p:Configuration=\"{$conf}\" /p:Platform=\"{$platform}\"";// > \"{$log_file}\"";
	}
	else {
		$cmds[0] = "%COMSPEC% /c \"\"{$exe}\" \"{$sln}\" /m /v:m /t:{$build} /p:PlatformToolset=v142 /p:Configuration=\"{$conf}\"";// > \"{$log_file}\"";
	}
	return $cmds;
}
